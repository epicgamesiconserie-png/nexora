      ...
      <div className="relative z-10 flex min-h-screen">
        <aside className="hidden md:flex w-64 flex-col p-4 border-r" style={{ background: "rgba(10, 10, 14, 0.85)", borderColor: "rgba(255, 255, 255, 0.06)", backdropFilter: "blur(20px)", WebkitBackdropFilter: "blur(20px)" }}>
          ...
          <nav className="flex flex-col gap-1 flex-1">
            ...
            <SidebarItem href="/dashboard/mypage" Icon={Home} label="My Page" />
        ...
        <div className="flex-1 min-w-0">
          <header className="sticky top-0 z-20 px-6 py-4 border-b" style={{ background: "rgba(10, 10, 14, 0.7)", borderColor: "rgba(255, 255, 255, 0.06)", backdropFilter: "blur(20px)", WebkitBackdropFilter: "blur(20px)" }}>
            <div className="flex items-center justify-between">
              <h1 className="text-lg font-bold tracking-tight" style={{ textShadow: "0 0 20px rgba(255,255,255,0.2)" }}>Dashboard</h1>
              <div className="flex items-center gap-3">
                <span className="text-sm text-zinc-400 hidden sm:block font-medium">@{user.username}</span>
                <Link href={`/u/${user.username}`} className="text-xs px-3 h-8 inline-flex items-center rounded-lg border border-white/15 hover:bg-white/5 transition font-semibold">View public page</Link>
              </div>
            </div>
          </header>
          ...
          <div className="p-6 md:p-8 max-w-7xl mx-auto">
            <div className="grid grid-cols-1 lg:grid-cols-[1fr_360px] gap-8">
              <div className="min-w-0">
                <div className="mb-10">
                  <h2 className="text-3xl md:text-4xl font-bold tracking-tight" style={{ textShadow: "0 1px 0 rgba(255,255,255,0.9), 0 2px 0 rgba(200,200,200,0.6), 0 4px 12px rgba(0,0,0,0.5), 0 0 30px rgba(255,255,255,0.35)" }}>Welcome, @{user.username}</h2>
                  <p className="text-zinc-400 mt-3 text-sm">Your account was created on {user.createdAt.toDateString()}.</p>
                </div>
                ...
                <div className="rounded-2xl border border-white/10 bg-white/[0.02] backdrop-blur-sm p-6 mb-8">
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="text-base font-bold" style={{ textShadow: "0 0 20px rgba(255,255,255,0.15)" }}>Profile Completion</h3>
                  ...
                  <div className="w-full h-2 rounded-full bg-white/5 overflow-hidden mb-6">
                    <div className="h-full rounded-full" style={{ width: "0%", background: "linear-gradient(90deg, #a855f7 0%, #c084fc 100%)", boxShadow: "0 0 20px rgba(168,85,247,0.6)" }} />
                  </div>
                  <div className="rounded-xl border border-amber-500/30 bg-amber-500/5 p-4 flex items-start gap-3 mb-6">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-zinc-400 mt-1">Complete your profile to make it more discoverable and appealing.</div>
                    </div>
                  </div>
...
function StatCard({ Icon, label, value, action }: { Icon: React.ElementType; label: string; value: string; action?: string }) {
  return (
    <div className="rounded-2xl border border-white/10 bg-white/[0.02] backdrop-blur-sm p-4 hover:border-white/20 transition" style={{ boxShadow: "0 0 30px rgba(255,255,255,0.02), inset 0 1px 0 rgba(255,255,255,0.03)" }}>
      <div className="flex items-center justify-between mb-3">
        <span className="text-xs uppercase tracking-wider text-zinc-500 font-bold">{label}</span>
        <Icon className="h-4 w-4 text-zinc-500" />
      </div>
      <div className="text-base font-bold truncate" style={{ textShadow: "0 0 12px rgba(255,255,255,0.2)" }}>{value}</div>
...
function ChecklistRow({ Icon, label, href, done }: { Icon: React.ElementType; label: string; href: string; done?: boolean }) {
  return (
    <Link href={href} className={`w-full flex items-center gap-3 rounded-xl border p-4 text-left transition ${done ? "border-emerald-500/30 bg-emerald-500/5" : "border-white/10 bg-white/[0.02] hover:bg-white/[0.05] hover:border-white/20"}`}>
      <Icon className="h-4 w-4 text-zinc-400" />
      <span className={`text-sm font-bold flex-1 ${done ? "text-emerald-300" : "text-white"}`} style={{ textShadow: "0 0 12px rgba(255,255,255,0.15)" }}>{label}</span>
